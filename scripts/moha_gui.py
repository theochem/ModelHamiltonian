...
def on_moltype_dropdown_select(event, ents):
...
if __name__ == '__main__':
    ...
    moltype_selected = tk.StringVar(value=mol_fields[0])
    moltype_dropdown = ttk.Combobox(row, values=mol_fields, textvariable=moltype_selected, width=6)
    ...
    moltype_dropdown.bind("<<ComboboxSelected>>", (lambda event, 
                                                   ents=mol_ents: 
                                                   on_moltype_dropdown_select(event, ents)))
